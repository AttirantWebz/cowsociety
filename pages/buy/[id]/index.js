...
export default function ListProduct({ list, negolist }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const list = await fetch(`/api/list/${id[2]}`).then((res) => res.json());
      if (list.success) {
        console.log("li", list);
        setproduct(list.list);
      }
      const negolist = await fetch(`/api/negotiateget?id=${id[2]}`).then(
        (res) => res.json()
      );
      if (negolist.success) {
          ...
          <Grid container className="buy-product">
            ...
            <Grid className="buy-body">
              <Paper elevation={0} className="buy-paper">
                ...
                <Link
                  className="buy-btn"
                  href={`/buy/${product[0]._id}/negotiate?id=${product[0]._id}`}
                >
                  <button>buy</button>
                </Link>
              </Paper>
